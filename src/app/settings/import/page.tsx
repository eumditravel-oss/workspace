    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-800">Excel Import 검증 (미리보기)</h1>
            <p className="text-xs text-gray-500">대량의 데이터를 시스템에 반영하기 전 오류와 매핑 상태를 확인합니다.</p>
          </div>
        </div>
        
        <div className="flex gap-2">
          <button className="px-4 py-2 border rounded-lg text-sm font-bold text-gray-700 hover:bg-gray-50">
            새 파일 업로드
          </button>
          <button 
            onClick={handleApply}
        ...
        <div className="grid grid-cols-4 gap-6">
          ...
          <div className="col-span-1 space-y-4">
            <div className="bg-white rounded-xl border shadow-sm p-4">
              <h3 className="font-bold text-gray-800 mb-4 border-b pb-2">세션 요약</h3>
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-500">파일</span>
                  <span className="font-medium text-gray-800 truncate ml-2" title={session.fileName}>{session.fileName}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">대상 시트</span>
                  <span className="font-medium text-gray-800">{session.targetSheet}</span>
                </div>
                <div className="flex justify-between">
            ...
            <div className="bg-white rounded-xl border shadow-sm p-2 flex flex-col gap-1">
              {[
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-4 gap-4">
                  <div className="bg-white p-4 rounded-xl border shadow-sm">
                    <div className="text-gray-500 text-xs font-bold mb-1">총 할당(일정) 건수</div>
                    <div className="text-2xl font-extrabold text-gray-800">{session.totalAssignments.toLocaleString()}</div>
                  </div>
                  <div className="bg-white p-4 rounded-xl border shadow-sm">
                    <div className="text-gray-500 text-xs font-bold mb-1">인식된 직원</div>
                    <div className="text-2xl font-extrabold text-gray-800">{session.totalPersonnel}</div>
                  </div>
                  <div className="bg-white p-4 rounded-xl border shadow-sm">
                    <div className="text-gray-500 text-xs font-bold mb-1">인식된 프로젝트</div>
                    <div className="text-2xl font-extrabold text-gray-800">{session.totalProjects}</div>
                  </div>
            ...
            {activeTab === 'ISSUES' && (
              <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
                <div className="p-4 border-b bg-gray-50 flex justify-between items-center">
                  <h3 className="font-bold text-gray-800">오류 및 경고 목록</h3>
                ...
                <div className="divide-y">
                  {sessionIssues.map(issue => (
                    <div key={issue.id} className={`p-4 transition-colors ${issue.status === 'RESOLVED' ? 'opacity-50 bg-gray-50' : 'hover:bg-gray-50'}`}>
                      <div className="flex gap-4">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between items-start">
                            <h4 className={`font-bold text-sm ${issue.severity === 'BLOCKER' ? 'text-red-700' : 'text-gray-800'}`}>
                              [{issue.issueType}] {issue.title}
                            </h4>
                            <div className="flex gap-2">
                              {issue.status === 'OPEN' && (
                                <>
                                  <button onClick={() => ignoreIssue(issue.id, currentUser.id)} className="text-xs px-2 py-1 rounded border text-gray-500 hover:bg-gray-100">무시</button>
                              ...
                              {issue.status !== 'OPEN' && (
                                <span className="text-xs font-bold text-gray-400 border px-2 py-1 rounded">{issue.status}</span>
                              )}
                            </div>
                          </div>
                  ...
                  {sessionIssues.length === 0 && (
                    <div className="p-12 text-center text-gray-500">발견된 오류나 경고가 없습니다.</div>
                  )}
                </div>
              </div>
            )}
