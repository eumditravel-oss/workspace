...
export default function BulkEditPage() {
  ...
  if (!['SUPER_ADMIN', 'SYSTEM_ADMIN'].includes(currentUser.role)) {
    return <div className="p-6 text-red-600 font-bold">운영 설정을 볼 권한이 없습니다. (관리자 전용)</div>;
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center gap-4 bg-white p-6 rounded-xl shadow-sm border">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-800">대량 수정 (Bulk Edit)</h1>
          <p className="text-sm text-gray-500 mt-1">Excel import 이후 파편화된 데이터(PM, 부서, 납품일, Scope 등)를 일괄 수정합니다.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-5 gap-6">
        <div className="col-span-2 space-y-6">
          <div className="bg-white p-6 rounded-xl border shadow-sm space-y-4">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">대상 엔티티</label>
                <select 
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">변경할 속성(필드)</label>
                <select 
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">변경될 새 값</label>
                <input 
                  type="text" 
            ...
            {!activePreview ? (
              ...
              <div className="flex gap-2 mt-2">
                ...
                <button 
                  ...
                  className="flex-1 py-2.5 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-lg font-bold transition-colors"
                >
                  취소
                </button>
              </div>
        ...
        <div className="col-span-3">
          {activePreview ? (
            <div className="bg-white p-6 rounded-xl border shadow-sm space-y-4">
              <h3 className="font-bold text-gray-800 border-b pb-2">변경 미리보기 (Preview)</h3>
              
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gray-50 p-4 rounded-lg border text-center">
                  <div className="text-sm text-gray-500 font-bold">전체 대상 항목</div>
                  <div className="text-3xl font-extrabold text-gray-800 mt-1">{activePreview.totalItems}</div>
                </div>
              ...
              <div className="mt-4 p-4 border rounded-lg bg-gray-50 text-sm text-gray-600 text-center">
                  ...
                  <div key={session.id} className="p-4 border rounded-lg flex justify-between items-center bg-gray-50">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`text-xs font-bold px-2 py-0.5 rounded ${session.status === 'APPLIED' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-200 text-gray-700'}`}>
                          {session.status}
                        </span>
                        <span className="text-sm font-bold text-gray-800">엔티티: {session.targetEntityType}</span>
                      </div>
