...
exports.approveAssignment = exports.rejectAssignment = exports.submitAssignment = exports.getTemplates = void 0;
const db_1 = require("../lib/db");
const zod_1 = require("zod");
const getTemplates = async (req, res) => {
    try {
        const templates = await db_1.prisma.processTemplate.findMany({
            include: {
                stages: {
                    include: {
                        tasks: true
...
const assignmentSubmitSchema = zod_1.z.object({
    ...
    schedules: zod_1.z.array(zod_1.z.any())
});
const submitAssignment = async (req, res) => {
    try {
        ...
        const pmId = req.user.personnelId;
...
const rejectAssignment = async (req, res) => {
    try {
        ...
        const managerId = req.user.personnelId;
        if (!rejectionReason)
            return res.status(400).json({ error: 'Rejection reason is required' });
        const assignment = await db_1.prisma.processTemplateAssignment.findUnique({ where: { id: assignmentId }, include: { schedules: true } });
