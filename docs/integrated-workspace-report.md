...
> [!WARNING]
> 본 리포트의 모든 개발 범위는 구현이 완료되어 `origin/main` (Commit: `dbd73ba`) 에 동기화되었으나, Lint 오류와 GitHub 기특수 라우팅 대응 등 잔존 리스크를 제거하기 전까지는 실 서비스 환경(프로덕션) 투입에 있어 제한적인 테스트 운영을 권장합니다.

---

## 11. 납품 상태 및 사후 추가업무 (Plan 14)

최근 업데이트를 통해 납품일 기반의 프로젝트 자동 상태 분류 및 납품 이후 추가 작업 요청 프로세스가 구현되었습니다.

- **납품일 경과 배지**: 프로젝트 보드에 당일, 납품일 전, 납품일 경과 상태를 시각적으로 표시합니다.
- **자동 완료 처리 기준**: 운영 설정에 따라 납품일이 지난 프로젝트는 진행률에 관계없이 "완료" 컬럼에 임시 자동 분류됩니다.
- **사후 추가업무 요청**: 완료된 프로젝트에 대해 클라이언트 추가 요청, 결함 수정 등의 사유로 추가업무를 PM에게 요청할 수 있는 플로우가 구현되었습니다.
- **프로젝트 재오픈**: PM 승인 시 추가업무 요청이 받아들여져 프로젝트가 다시 재개(REOPEN) 상태로 전환됩니다.
- **데이터 품질 검사 (Data Quality Checks)**: 완료 프로젝트에 남은 미결 결재/요청건이 있거나 잘못된 납품 상태 표시가 있는 경우 시스템이 경고를 띄웁니다.
- **운영 설정 (Workspace Settings)**: 관리자는 `설정` 페이지에서 위험 작업인 "일괄 마감 미리보기 및 실행" 등 자동 완료에 대한 일괄 적용 정책을 관리할 수 있습니다.
